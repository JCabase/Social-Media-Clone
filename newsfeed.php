...
<html>
  <head>
    <title>Social Media Clone - Newsfeed</title>    
    <meta charset="utf-8">
        ...
        <div class="row" style="padding-top: 25px;padding-bottom: 25px;">
                        ...
                        <div>  
                          <?php 
                              if (!$row['user_profile_image']){
                                ...
                                echo "<img src = 'images/" . $row['user_profile_image']."'>";
                              }
                          ?>
                        </div>
                      <h5><?php echo $row['user_firstname']." ".$row['user_lastname'] ; ?></h5>
                        <?php } ?>
                    </div>
            ...
            <div class="col-sm-9">
                ...
                <?php
                  ...
                  $query = "SELECT *, DATE_FORMAT(`time_post`, '%d/%m/%Y %h:%i %p') as `datetime_post` FROM `post_table` a LEFT JOIN `user_table` b ON b.ID = a.user_id ORDER BY `time_post` DESC";
               ...
               ?>                         
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <p class="text-muted mb-2"><?php echo $row['datetime_post'];?></p>
                        <?php 
                    ...
                    <div class="card-footer">
                      <?php 
                            echo "<button class='btn btn-danger text-white mb-2' type='submit' name = 'btnlike' value = '".$row['post_id']."'>Unlike</button>";
                        }
                        else {
                          echo "<button class='btn btn-info text-white mb-2' type='submit' name = 'btnlike' value = '".$row['post_id']."'>Like</button>";
                            ...
                            while($rowlike = mysqli_fetch_array($resultlike)){
                              ...
                              echo "<p>".$rowlike['user_firstname'], " ", $rowlike['user_lastname'], " commented: "."";
                              echo "</p>";
                              echo "<input type = 'text' class='form-control my-2' name = 'edit_comment' id = 'edittext' disabled='true' value = '".$rowlike['comment_text']."'>";
                              ...
                              if(($getusercommenttodelete == $userID) && ($getpostidfromcommentable == $getcomment)) {
                                echo "<button type='submit' class='btn btn-danger my-2' style='margin-right: 10px;' name = 'btndelete_comment' value = '".$rowlike['comment_id']."'>Delete</button>";
                                echo "<a href='edit_comment.php?id=".$rowlike['comment_id']."' class='btn btn-warning my-2 text-white'>Edit</a>";
                            ...
                            echo "<button type='submit' name = 'btncomment' value = '".$row['post_id']."' class='btn btn-success'>Comment</button>";
                      ?>
